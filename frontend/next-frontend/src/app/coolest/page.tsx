...
import CoolestClient from "./client";
...
export default async function CoolestPage() {
  ...
  try {
    ...
    // Get products with category for coolest gadgets (using categoryId as a number)
    const goodsResponse = await serverAxios.get(goodsApiPath, {
      params: {
        pageNum: 1,
        pageSize: 100, // Fetch up to 100 items, client will handle pagination
        categoryId: 6, // Changed from "CoolStuff" to numeric value 1
        siteId: siteId,
      },
    });
    ...
    return <CoolestClient coolProducts={goodsData} />;
  } catch (error) {
    console.error("API request error:", error);
    return (
      <div className="container mx-auto px-4 py-8 text-center">
        <h1 className="text-3xl font-bold mb-4">Coolest Gadgets</h1>
